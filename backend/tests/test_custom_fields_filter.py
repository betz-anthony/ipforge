...
def test_subnet_tag_filter(client, db):
    ...
    db.add_all([a, b])
    db.commit()
    client.put(f"/api/v1/subnets/{a.id}", json={"tags": ["prod"]})

    rows = client.get("/api/v1/subnets", params={"tag": "prod"}).json()
...
def test_subnet_custom_field_filter(client, db):
    ...
    client.put(f"/api/v1/subnets/{a.id}", json={"custom_fields": {"env": "prod"}})
    client.put(f"/api/v1/subnets/{b.id}", json={"custom_fields": {"env": "dev"}})

    rows = client.get("/api/v1/subnets", params={"cf_env": "prod"}).json()
...
def test_subnet_combined_tag_and_field_filter(client, db):
    ...
    db.commit()
    _field(db, "subnet", "env")
    client.put(f"/api/v1/subnets/{a.id}", json={"custom_fields": {"env": "prod"}, "tags": ["x"]})
    client.put(f"/api/v1/subnets/{b.id}", json={"custom_fields": {"env": "prod"}})

    rows = client.get("/api/v1/subnets", params={"tag": "x", "cf_env": "prod"}).json()
    assert {r["id"] for r in rows} == {a.id}
...
def test_address_tag_filter(client, db):
    ...
    db.add_all([a1, a2])
    db.commit()
    client.put(f"/api/v1/addresses/{a1.id}", json={"tags": ["managed"]})

    rows = client.get("/api/v1/addresses", params={"tag": "managed"}).json()["items"]
...
def test_subnet_csv_includes_custom_fields_and_tags(client, db):
    ...
    client.put(f"/api/v1/subnets/{s.id}", json={"custom_fields": {"owner": "alice"}, "tags": ["critical"]})
