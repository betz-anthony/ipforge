...
def test_upsert_and_list_policy(client, db):
    r = client.put("/api/v1/drift/policies/orphan_dhcp", json={"mode": "auto", "dry_run": True})
    assert r.status_code == 200, r.text
    rows = client.get("/api/v1/drift/policies").json()
    assert any(p["category"] == "orphan_dhcp" and p["mode"] == "auto" for p in rows)


def test_auto_rejected_for_unsafe_category(client, db):
    r = client.put("/api/v1/drift/policies/multi_dhcp_scope", json={"mode": "auto"})
    assert r.status_code == 400


def test_review_allowed_for_unsafe(client, db):
    r = client.put("/api/v1/drift/policies/multi_dhcp_scope", json={"mode": "review"})
    assert r.status_code == 200


def test_invalid_target_status(client, db):
    r = client.put("/api/v1/drift/policies/active_but_available",
                   json={"mode": "auto", "params": {"target_status": "bogus"}})
    assert r.status_code == 400


def test_delete_policy(client, db):
    client.put("/api/v1/drift/policies/orphan_dns", json={"mode": "auto"})
    assert client.delete("/api/v1/drift/policies/orphan_dns").status_code == 204
    assert db.query(DriftPolicy).filter_by(category="orphan_dns").count() == 0


def test_policy_requires_admin(client_operator, db):
    r = client_operator.put("/api/v1/drift/policies/orphan_dhcp", json={"mode": "auto"})
    assert r.status_code == 403


def test_policy_with_provider_action_allowed_auto(client, db):
    r = client.put("/api/v1/drift/policies/missing_dns",
...
def test_subnet_policy_create_and_list(client, db):
    ...
    db.add(s); db.commit()
    r = client.put("/api/v1/drift/policies/orphan_dhcp",
...
def test_subnet_policy_overrides_global_in_list(client, db):
    ...
    s = Subnet(name="T", cidr="10.1.0.0/24", ip_version=4)
    db.add(s); db.commit()
    client.put("/api/v1/drift/policies/orphan_dhcp", json={"mode": "review"})
    client.put("/api/v1/drift/policies/orphan_dhcp", json={"mode": "auto", "subnet_id": s.id})
    rows = client.get("/api/v1/drift/policies").json()
...
def test_delete_subnet_policy(client, db):
    ...
    db.add(s); db.commit()
    client.put("/api/v1/drift/policies/orphan_dhcp", json={"mode": "auto", "subnet_id": s.id})
    r = client.delete(f"/api/v1/drift/policies/orphan_dhcp?subnet_id={s.id}")
...
def test_needs_review_filter(client, db):
    ...
    db.commit()
    rows = client.get("/api/v1/drift", params={"needs_review": "true"}).json()
