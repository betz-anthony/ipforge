...
def test_create_and_list_range(client, db):
    s = _subnet(db)
    r = client.post(f"/api/subnets/{s.id}/ranges", json={
        "start_ip": "10.0.0.10", "end_ip": "10.0.0.20", "kind": "dhcp_pool", "label": "pool",
    })
    assert r.status_code == 201, r.text
    rid = r.json()["id"]

    rows = client.get(f"/api/subnets/{s.id}/ranges").json()
...
def test_range_overlap_conflict(client, db):
    s = _subnet(db)
    client.post(f"/api/subnets/{s.id}/ranges", json={"start_ip": "10.0.0.10", "end_ip": "10.0.0.20", "kind": "reserved"})
    r = client.post(f"/api/subnets/{s.id}/ranges", json={"start_ip": "10.0.0.15", "end_ip": "10.0.0.25", "kind": "reserved"})
...
def test_delete_range(client, db):
    ...
    assert client.delete(f"/api/subnets/{s.id}/ranges/{rid}").status_code == 204
    assert db.query(SubnetRange).count() == 0


def test_range_create_requires_operator(client_gr, db):
    s = _subnet(db)
    r = client_gr.post(f"/api/subnets/{s.id}/ranges", json={
