...
def test_plan_endpoint(client, db):
    r = client.post("/api/gitops/plan", content=DOC, headers={"content-type": "text/yaml"})
...
def test_apply_endpoint(client, db):
    r = client.post("/api/gitops/apply", content=DOC, headers={"content-type": "text/yaml"})
    assert r.status_code == 200, r.text
...
def test_bad_yaml_400(client, db):
    r = client.post("/api/gitops/apply", content="subnets: [", headers={"content-type": "text/yaml"})
    assert r.status_code == 400


def test_missing_source_400(client, db):
    r = client.post("/api/gitops/plan", content="subnets: []", headers={"content-type": "text/yaml"})
    assert r.status_code == 400


def test_requires_operator(client_gr, db):
    r = client_gr.post("/api/gitops/plan", content=DOC, headers={"content-type": "text/yaml"})
