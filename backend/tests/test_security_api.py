...
def test_list_and_filter(client, db):
    ...
    assert len(client.get("/api/v1/security/events").json()) == 2
    only = client.get("/api/v1/security/events", params={"event_type": "ip_conflict"}).json()
    assert [e["event_type"] for e in only] == ["ip_conflict"]


def test_acknowledge(client, db):
    e = _evt(db)
    assert client.post(f"/api/v1/security/events/{e.id}/ack").status_code == 200
    db.refresh(e)
...
def test_quarantine_imports_tags_sets_status(client, db):
    ...
    r = client.post(f"/api/v1/security/events/{e.id}/quarantine")
...
def test_quarantine_existing_address(client, db):
    ...
    db.add(a); db.commit()
    e = _evt(db, ip="10.0.0.50")
    client.post(f"/api/v1/security/events/{e.id}/quarantine")
    db.refresh(a)
...
def test_quarantine_requires_operator(client_gr, db):
    e = _evt(db)
    assert client_gr.post(f"/api/v1/security/events/{e.id}/quarantine").status_code == 403


def test_ack_requires_operator(client_gr, db):
    e = _evt(db)
    assert client_gr.post(f"/api/v1/security/events/{e.id}/ack").status_code == 403
