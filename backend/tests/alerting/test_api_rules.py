def test_create_rule(client_admin):
    ch_id = client_admin.post("/api/v1/alerts/channels",
                              json={"name": "c", "kind": "generic", "config": {"url": "u"}}).json()["id"]
    r = client_admin.post("/api/v1/alerts/rules", json={
...
def test_list_update_delete_rule(client_admin):
    ch_id = client_admin.post("/api/v1/alerts/channels",
                              json={"name": "c", "kind": "generic", "config": {"url": "u"}}).json()["id"]
    rid = client_admin.post("/api/v1/alerts/rules", json={"name": "r", "trigger_type": "sync_error",
                                                        "channel_ids": [ch_id], "condition": {},
                                                        "recipients": []}).json()["id"]
    assert client_admin.get("/api/v1/alerts/rules").status_code == 200
    upd = client_admin.put(f"/api/v1/alerts/rules/{rid}",
                           json={"name": "r2", "trigger_type": "sync_error", "channel_ids": [ch_id],
                                 "condition": {}, "recipients": [], "enabled": False})
    assert upd.json()["enabled"] is False
    assert client_admin.delete(f"/api/v1/alerts/rules/{rid}").status_code == 204


def test_rule_duplicate_name_409(client_admin):
    ch_id = client_admin.post("/api/v1/alerts/channels",
                              json={"name": "c", "kind": "generic", "config": {"url": "u"}}).json()["id"]
    client_admin.post("/api/v1/alerts/rules", json={"name": "dup", "trigger_type": "collision",
                                                  "channel_ids": [ch_id], "recipients": [], "condition": {}})
    r = client_admin.post("/api/v1/alerts/rules", json={"name": "dup", "trigger_type": "sync_error",
                                                      "channel_ids": [ch_id], "recipients": [], "condition": {}})
...
def test_update_rule_rejects_rename_to_existing(client_admin):
    ch_id = client_admin.post("/api/v1/alerts/channels",
                              json={"name": "c", "kind": "generic", "config": {"url": "u"}}).json()["id"]
    a = client_admin.post("/api/v1/alerts/rules", json={"name": "a", "trigger_type": "collision",
                                                      "channel_ids": [ch_id], "recipients": [], "condition": {}}).json()
    b = client_admin.post("/api/v1/alerts/rules", json={"name": "b", "trigger_type": "collision",
                                                      "channel_ids": [ch_id], "recipients": [], "condition": {}}).json()
    r = client_admin.put(f"/api/v1/alerts/rules/{b['id']}", json={
...
def test_non_admin_forbidden_on_rules(client_operator):
    r = client_operator.post("/api/v1/alerts/rules", json={"name": "x", "trigger_type": "collision",
                                                         "channel_ids": [], "recipients": [], "condition": {}})
    assert r.status_code == 403


def test_list_rules_operator_403(client_operator):
    assert client_operator.get("/api/v1/alerts/rules").status_code == 403


def test_list_rules_readonly_403(client_gr):
    assert client_gr.get("/api/v1/alerts/rules").status_code == 403


def test_list_rules_scoped_403(client_scoped):
    assert client_scoped.get("/api/v1/alerts/rules").status_code == 403
