def test_notes_field_in_address_response(client):
    r = client.post("/api/subnets", json={"name": "Net", "cidr": "10.0.0.0/24"})
    assert r.status_code == 201
    subnet_id = r.json()["id"]

    r = client.post("/api/addresses", json={"address": "10.0.0.1", "subnet_id": subnet_id})
    assert r.status_code == 201
...
def test_update_address_notes(client):
    ...
    r = client.put(f"/api/addresses/{addr_id}", json={"notes": "rack 3 slot 2"})
...
def test_update_subnet_notes(client):
    ...
    r = client.put(f"/api/subnets/{subnet_id}", json={"notes": "data center A"})
    assert r.status_code == 200
...
def test_get_address_by_ip(client):
    ...
    r = client.get("/api/addresses/by-ip/10.0.0.5")
    assert r.status_code == 200
...
def test_get_address_by_ip_not_found(client):
    r = client.get("/api/addresses/by-ip/192.168.99.99")
