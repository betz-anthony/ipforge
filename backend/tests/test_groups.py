...
def test_create_and_list_group(client, db):
    r = client.post("/api/groups", json={"name": "netops", "description": "Net ops"})
...
def test_update_and_delete_group(client, db):
    gid = client.post("/api/groups", json={"name": "tmp"}).json()["id"]
    assert client.put(f"/api/groups/{gid}", json={"description": "x"}).status_code == 200
...
def test_add_and_remove_member(client, db):
    gid = client.post("/api/groups", json={"name": "team"}).json()["id"]
    user = User(username="member", hashed_password="x", role="scoped", enabled=True)
    db.add(user)
    db.commit()
    db.refresh(user)

    r = client.post(f"/api/groups/{gid}/members", json={"user_id": user.id})
    assert r.status_code == 204
    members = client.get(f"/api/groups/{gid}/members").json()
    assert any(m["id"] == user.id for m in members)

    r = client.request("DELETE", f"/api/groups/{gid}/members", json={"user_id": user.id})
    assert r.status_code == 204
    assert client.get(f"/api/groups/{gid}/members").json() == []
