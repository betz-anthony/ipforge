...
def test_list_addresses_q_ilike(client, db):
    ...
    r = client.get("/api/addresses?q=prod")
    body = r.json()
    assert body["total"] == 2
...
def test_list_addresses_q_matches_ip(client, db):
    ...
    r = client.get("/api/addresses?q=10.0.0.99")
    body = r.json()
    assert body["total"] == 1
...
def test_list_addresses_sort_hostname_asc(client, db):
    ...
    r = client.get("/api/addresses?sort=hostname&dir=asc")
    items = r.json()["items"]
...
def test_list_addresses_sort_hostname_desc(client, db):
    ...
    r = client.get("/api/addresses?sort=hostname&dir=desc")
    items = r.json()["items"]
    hostnames = [a["hostname"] for a in items]
...
def test_list_addresses_offset_window(client, db):
    ...
    r = client.get("/api/addresses?sort=address&dir=asc&limit=2&offset=2")
    body = r.json()
    assert body["total"] == 5
...
def test_list_addresses_total_honors_filters(client, db):
    ...
    r = client.get("/api/addresses?status=assigned")
    body = r.json()
    assert body["total"] == 2
...
def test_list_addresses_unknown_sort_ignored(client, db):
    sn = _subnet(db)
    _addr(db, sn.id, "10.0.0.1")
    r = client.get("/api/addresses?sort=injected_column&dir=asc")
    assert r.status_code == 200
    body = r.json()
    assert "items" in body
