...
def test_create_and_list(client, db):
    r = client.post("/api/v1/automation/rules", json={
        "name": "tag-rogue", "trigger_type": "rogue", "action": {"add_tags": ["unverified"]},
    })
    assert r.status_code == 201, r.text
    rid = r.json()["id"]
    assert any(x["id"] == rid for x in client.get("/api/v1/automation/rules").json())
...
def test_update_and_delete(client, db):
    rid = client.post("/api/v1/automation/rules", json={
        "name": "r", "trigger_type": "drift", "action": {"add_tags": ["a"]},
    }).json()["id"]
    assert client.put(f"/api/v1/automation/rules/{rid}", json={"enabled": False}).status_code == 200
    assert db.get(AutomationRule, rid).enabled is False
    assert client.delete(f"/api/v1/automation/rules/{rid}").status_code == 204
    assert db.get(AutomationRule, rid) is None


def test_requires_admin(client_operator, db):
    r = client_operator.post("/api/v1/automation/rules", json={
