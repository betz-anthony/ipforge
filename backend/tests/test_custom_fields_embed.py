...
def test_subnet_put_and_read_custom_fields_and_tags(client, db):
    ...
    r = client.put(f"/api/v1/subnets/{s.id}", json={
...
def test_subnet_unknown_field_rejected(client, db):
    ...
    db.add(s)
    db.commit()
    r = client.put(f"/api/v1/subnets/{s.id}", json={"custom_fields": {"nope": "x"}})
...
def test_subnet_select_validation(client, db):
    ...
    assert client.put(f"/api/v1/subnets/{s.id}", json={"custom_fields": {"env": "stage"}}).status_code == 400
    assert client.put(f"/api/v1/subnets/{s.id}", json={"custom_fields": {"env": "prod"}}).status_code == 200
...
def test_subnet_clear_field_with_empty(client, db):
    ...
    db.commit()
    _mkfield(db, "subnet", "owner")
    client.put(f"/api/v1/subnets/{s.id}", json={"custom_fields": {"owner": "bob"}})
    client.put(f"/api/v1/subnets/{s.id}", json={"custom_fields": {"owner": ""}})
    row = next(x for x in client.get("/api/v1/subnets").json() if x["id"] == s.id)
...
def test_address_put_and_read_custom_fields_and_tags(client, db):
    ...
    r = client.put(f"/api/v1/addresses/{a.id}", json={
        "custom_fields": {"role": "db"}, "tags": ["managed"],
    })
    assert r.status_code == 200, r.text

    row = next(x for x in client.get("/api/v1/addresses").json()["items"] if x["id"] == a.id)
