...
def test_create_and_list_field_def(client, db):
    ...
    r = client.get("/api/v1/custom-fields", params={"entity_type": "subnet"})
    assert r.status_code == 200
    names = [f["name"] for f in r.json()]
    assert "owner" in names

    # filtered out for other entity type
    r = client.get("/api/v1/custom-fields", params={"entity_type": "address"})
...
def test_duplicate_name_same_entity_conflict(client, db):
    ...
    assert client.post("/api/v1/custom-fields", json=body).status_code == 201
    assert client.post("/api/v1/custom-fields", json=body).status_code == 409
...
def test_delete_field_cascades_values(client, db):
    ...
    r = client.delete(f"/api/v1/custom-fields/{fid}")
...
def test_non_admin_cannot_create_field(client_operator, db):
    r = client_operator.post("/api/v1/custom-fields", json={
        "entity_type": "subnet", "name": "owner", "label": "Owner", "field_type": "text",
    })
...
def test_tags_create_list_delete(client, db):
    r = client.post("/api/v1/tags", json={"name": "critical"})
    assert r.status_code == 201
    tid = r.json()["id"]
    ...
    # duplicate
    assert client.post("/api/v1/tags", json={"name": "critical"}).status_code == 409
...
def test_operator_can_create_tag(client_operator, db):
    r = client_operator.post("/api/v1/tags", json={"name": "new"})
    assert r.status_code == 201
