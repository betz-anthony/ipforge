...
def test_submit_request_as_requester(client_requester, db):
    ...
    db.add(s); db.commit()
    r = client_requester.post("/api/v1/requests", json={
...
def test_submit_duplicate_pending_409(client_requester, db):
    ...
    assert client_requester.post("/api/v1/requests", json=body).status_code == 201
    r2 = client_requester.post("/api/v1/requests", json=body)
...
def test_submit_scoped_403(client_scoped, db):
    ...
    s = Subnet(cidr="10.0.0.0/29", name="t", request_eligible=True)
    db.add(s); db.commit()
    r = client_scoped.post("/api/v1/requests", json={
        "subnet_id": s.id, "hostname": "host", "purpose": "valid purpose here",
    })
...
def test_list_requester_sees_only_own(client_requester, client_admin, db):
    ...
    s = Subnet(cidr="10.0.0.0/29", name="t", request_eligible=True)
    db.add(s); db.commit()
    client_requester.post("/api/v1/requests", json={
        "subnet_id": s.id, "hostname": "rhost", "purpose": "valid purpose here",
    })
    client_admin.post("/api/v1/requests", json={
        "subnet_id": s.id, "hostname": "ahost", "purpose": "valid purpose here",
    })
    r = client_requester.get("/api/v1/requests")
...
def test_list_admin_sees_all(client_requester, client_admin, db):
    ...
    r = client_admin.get("/api/v1/requests")
    assert r.status_code == 200
    assert len(r.json()) == 2
...
def test_list_filter_by_status(client_admin, db):
    ...
    r = client_admin.get("/api/v1/requests?status=pending")
    assert r.status_code == 200
    assert all(req["status"] == "pending" for req in r.json())


def test_list_scoped_403(client_scoped):
    assert client_scoped.get("/api/v1/requests").status_code == 403
...
def test_eligible_subnets_filters(client_requester, db):
    ...
    db.commit()
    r = client_requester.get("/api/v1/requests/eligible-subnets")
    assert r.status_code == 200
    body = r.json()
...
def test_get_one_requester_own_only(client_requester, client_admin, db):
    ...
    s = Subnet(cidr="10.0.0.0/29", name="t", request_eligible=True)
    db.add(s); db.commit()
    rid = client_admin.post("/api/v1/requests", json={
        "subnet_id": s.id, "hostname": "admin-req", "purpose": "valid purpose here",
    }).json()["id"]
    assert client_requester.get(f"/api/v1/requests/{rid}").status_code == 403
...
def test_list_invalid_status_422(client_admin):
    assert client_admin.get("/api/v1/requests?status=garbage").status_code == 422
...
def test_submit_as_readonly(client_gr, db):
    ...
    s = Subnet(cidr="10.0.0.0/29", name="t", request_eligible=True)
    db.add(s); db.commit()
    r = client_gr.post("/api/v1/requests", json={
...
def test_approve_success(client_admin, client_requester, db):
    ...
    s = Subnet(cidr="10.0.0.0/29", name="t", request_eligible=True)
    db.add(s); db.commit()
    rid = client_requester.post("/api/v1/requests", json={
        "subnet_id": s.id, "hostname": "newhost", "purpose": "valid purpose here",
    }).json()["id"]
    r = client_admin.put(f"/api/v1/requests/{rid}/approve", json={
...
def test_approve_not_pending_409(client_admin, client_requester, db):
    ...
    assert client_admin.put(f"/api/v1/requests/{rid}/approve", json={}).status_code == 200
    r2 = client_admin.put(f"/api/v1/requests/{rid}/approve", json={})
    assert r2.status_code == 409
...
def test_approve_requires_operator(client_requester, client_gr, db):
    ...
    assert client_gr.put(f"/api/v1/requests/{rid}/approve", json={}).status_code == 403
    assert client_requester.put(f"/api/v1/requests/{rid}/approve", json={}).status_code == 403
...
def test_approve_emits_resolved(client_admin, client_requester, db):
    ...
    with patch("app.api.ip_requests.emit") as e:
        client_admin.put(f"/api/v1/requests/{rid}/approve", json={})
...
def test_deny_success(client_admin, client_requester, db):
    ...
    r = client_admin.put(f"/api/v1/requests/{rid}/deny", json={"review_notes": "not approved — bad purpose"})
    assert r.status_code == 200
    body = r.json()
...
def test_deny_requires_notes(client_admin, client_requester, db):
    ...
    r = client_admin.put(f"/api/v1/requests/{rid}/deny", json={"review_notes": ""})
    assert r.status_code == 422
...
def test_deny_not_pending_409(client_admin, client_requester, db):
    ...
    client_admin.put(f"/api/v1/requests/{rid}/deny", json={"review_notes": "no"})
    r2 = client_admin.put(f"/api/v1/requests/{rid}/deny", json={"review_notes": "still no"})
    assert r2.status_code == 409
...
def test_delete_own_pending(client_requester, db):
    ...
    assert client_requester.delete(f"/api/v1/requests/{rid}").status_code == 204
...
def test_delete_requester_cant_delete_others(client_requester, client_admin, db):
    ...
    assert client_requester.delete(f"/api/v1/requests/{rid}").status_code == 403
...
def test_delete_operator_can_delete_any(client_operator, client_requester, db):
    ...
    assert client_operator.delete(f"/api/v1/requests/{rid}").status_code == 204
...
def test_deny_requires_operator(client_gr, client_requester, db):
    ...
    assert client_gr.put(f"/api/v1/requests/{rid}/deny", json={"review_notes": "x"}).status_code == 403
    assert client_requester.put(f"/api/v1/requests/{rid}/deny", json={"review_notes": "x"}).status_code == 403
...
def test_delete_readonly_403(client_gr, client_admin, db):
    ...
    assert client_gr.delete(f"/api/v1/requests/{rid}").status_code == 403


def test_delete_not_found_404(client_admin):
    assert client_admin.delete("/api/v1/requests/99999").status_code == 404
